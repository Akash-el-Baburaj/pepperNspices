  ...
  template: `
    <div class="min-h-screen bg-cinnamon-50 pt-32 pb-24 flex items-center justify-center">
      <div class="max-w-4xl w-full mx-auto px-4">
        ...
        <!-- SPLIT PANEL CARD -->
        <div class="bg-white border border-cinnamon-100 rounded-3xl overflow-hidden shadow-md grid grid-cols-1 md:grid-cols-2 min-h-[550px]">
          ...
          <div class="relative hidden md:block bg-peppercorn-950 overflow-hidden select-none">
            ...
            <div class="absolute inset-0 p-10 flex flex-col justify-between text-white z-10">
              ...
              <div class="space-y-4">
                <h2 class="text-3xl font-bold font-display leading-tight">Join the Circle of Sourcing</h2>
                <p class="text-xs text-cinnamon-100 leading-relaxed font-medium">
                  Create an account to track your orders, save curated spice wishlists, and receive alerts for seasonal crop harvests.
                </p>
              </div>
          ...
          <div class="p-8 md:p-12 flex flex-col justify-center bg-kraft-paper relative">
            <div class="mb-6">
              <h1 class="text-2xl font-extrabold text-peppercorn-950 font-display">Create Account</h1>
              <p class="text-xs text-peppercorn-500 font-medium mt-1">Begin your culinary journey with us.</p>
            </div>
            ...
            <form (submit)="onSubmit()" class="space-y-4">
              ...
              <div class="space-y-1">
                ...
                @if (password.length > 0) {
                  <div class="pt-2 space-y-1 animate-fade-in">
                    <div class="flex gap-1 h-1.5 rounded-full overflow-hidden bg-cinnamon-150">
                      @for (seg of [1, 2, 3, 4]; track seg) {
                        <div 
                          [class.bg-red-500]="seg <= strengthScore() && strengthScore() === 1"
                          [class.bg-orange-500]="seg <= strengthScore() && strengthScore() === 2"
                          [class.bg-yellow-500]="seg <= strengthScore() && strengthScore() === 3"
                          [class.bg-emerald-600]="seg <= strengthScore() && strengthScore() === 4"
                          [class.bg-transparent]="seg > strengthScore()"
                          class="flex-grow transition-all duration-300"
                        ></div>
                      }
                    </div>
                    <span class="text-[9px] font-bold uppercase tracking-wider block" [class]="strengthColorClass()">
              ...
              <!-- Confirm Password -->
              <div class="space-y-1">
                <label class="text-[10px] font-bold text-peppercorn-500 uppercase">Confirm Password</label>
                <input 
                  type="password" 
            ...
            <div class="relative flex items-center justify-center my-6">
              ...
              <span class="relative bg-white px-3 text-[9px] font-bold text-peppercorn-400 uppercase tracking-widest">Or sign up with</span>
            </div>
            ...
            <p class="text-xs text-peppercorn-500 font-medium text-center mt-6">
              ...
              <a routerLink="/login" class="font-bold text-chili-600 hover:text-chili-700 underline">Login here</a>.
            </p>
          </div>
...
export class RegisterComponent {
  ...
  protected strengthScore = signal(0);
  ...
  protected strengthColorClass = computed(() => {
    const score = this.strengthScore();
  ...
  onPasswordChange() {
    ...
    if (/\d/.test(p)) score += 1; // numbers
    if (/[A-Z]/.test(p) && /[a-z]/.test(p)) score += 1; // mixed case
    if (/[^A-Za-z0-9]/.test(p)) score += 1; // special chars

    this.strengthScore.set(score === 0 ? 1 : score);
  }
